...
formheight = 33; // added if we want to create a form within the SVG
...
d3.json("lda.json", function(error, data) {
    ...
    // Select the 'forms' div and create the select topic input and the lambda input:
    var topicform = d3.select("#forms").append("form")
	.attr("id", "topicForm");

    var tinput = tform.append("input")
	.attr("type", "text")
	.attr("value", "Enter Topic Number");
    // Later we could create the form inside the SVG using the foreignObject element
    ...
    var svg = d3.select("#lda").append("svg")
        ...
        .attr("height", formheight + mdsheight + 2*margin.top + margin.bottom);

    // Create a group for the topic selection form:
    var selectTopic = svg.append("g")
	.attr("id", "selectTopic")
        .attr("transform", "translate(" + margin.left + "," + margin.top + ")"); 

    // Create a rectangle with background color to denote size of this group:
    selectTopic.append("rect")
    	.attr("x", 0)
    	.attr("y", 0)
    ...
    // Insert foreignObject here?

    // Create a group for the topic selection form:
    var selectLambda = svg.append("g")
	.attr("id", "selectLambda")
        .attr("transform", "translate(" + +(margin.left + mdswidth + termwidth) + "," + margin.top + ")"); 

    // Create a rectangle with background color to denote size of this group:
    selectLambda.append("rect")
	.attr("x", 0)
	.attr("y", 0)
        .attr("height", formheight) 
        .attr("width", mdswidth)
    ...
    var mdsplot = svg.append("g")
        .attr("id", "leftpanel")
        .attr("class", "points")
        .attr("transform", "translate(" + margin.left + "," + +(2*margin.top + formheight) + ")"); 
    ...
    var chart = svg.append("g")
        .attr("transform", "translate(" + +(margin.left + mdswidth + termwidth) + "," + +(2*margin.top + formheight) + ")")
        .attr("id", "bar-freqs");
