#' Check the coherence of the input objects, and sort terms and topics in decreasing order of frequency
#'
...
#' sorts the vocabulary in decreasing order of term frequency, and the topics in decreasing
#' order of topic proportion.
#' 
#' @param K the number of topics in the model
#' @param W the number of terms in the vocabulary
#' @param phi a matrix with W rows, one for each term in the vocabulary, and K columns, one for each topic,
#' where each column sums to one. Each column is the multinomial distribution over terms for a given topic in
#' an LDA topic model.
#' @param term.frequency an integer vector of length W containing the frequency of each term in the
#' vocabulary.
#' @param vocab a character vector of length W containing the unique terms in the corpus.
...
#' of \code{phi} must be in the same order as \code{term.frequency} and \code{vocab}. The vector
...
#' x <- check.inputs(K=40, W=10473, phi=APdata$phi, term.frequency=APdata$term.frequency,
#'                   vocab=APdata$vocab, topic.proportion=APdata$topic.proportion)
#' 

check.inputs <- function(K=integer(), W=integer(), phi=matrix(), term.frequency=integer(),
  ...
  # order rows of phi, term.frequency, and vocabulary in decreasing order of term.frequency:
