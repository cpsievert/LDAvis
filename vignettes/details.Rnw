...
\item $n_d$ tokens in document $d$, for $d = 1...D$
\item $N = \sum_d n_d$ total tokens in the corpus
\item $W$ terms in the vocabulary
\item $M_w$ is defined as the frequency of term $w$ across the corpus, where $\sum_w M_w = N$. 
\end{itemize}
...
Our interactive visualization tool, $\texttt{LDAvis}$, requires six input arguments:
\begin{enumerate}
...
\item $n_d$, the number of tokens observed in document $d$, where $n_d$ is required to be an integer greater than zero, for documents $d = 1...D$.
\item $M_w$, the frequency of term $w$ across the entire corpus, where $M_w$ is required to be an integer greater than zero for each term $w=1...W$.
\end{enumerate}
In general, $\boldsymbol\alpha$ and $\boldsymbol\beta$ are specified by the modeler, $n_d$ and $M_w$ are computed from the data, and the algorithm used to fit the model produces estimates of $\boldsymbol\phi$ and $\boldsymbol\theta$. When using the collapsed Gibbs sampler, we recommend using equations 6 and 7 from Griffiths and Steyvers (PNAS, 2004) to estimate $\boldsymbol\phi$ and $\boldsymbol\theta$, and we recommend averaging these estimates over multiple iterations of the MCMC after convergence.
...
\begin{enumerate}
\item \textbf{Default Topic Circles:} $K$ circles, one to represent each topic, whose areas are set to be proportional to the frequencies of the topics across the $N$ total tokens in the corpus. The default topic circles are displayed when no term is highlighted.
\item \textbf{Pink Bars:} $K \times W$ pink horizontal bars, each of which represents the estimated number of times a given term was generated by a given topic. When a topic is selected, we show the pink bars for the $R$ most \emph{relevant} terms for the selected topic, where $R = 30$ by default.
\item \textbf{Gray Bars:} $W$ gray horizontal bars, one to represent the overall frequency of each term in the corpus. When no topic is selected, we display the gray bars for the $R$ most salient terms in the corpus, and when a topic is selected, we display the gray bars for the $R$ most relevant terms.
\item \textbf{Topic-Term Circles:} $K \times W$ circles whose areas are set to be proportional to the frequencies with which a given term is estimated to have been generated by each topic. When a given term, $w$, is highlighted, the $K$ default circles transition (i.e. their areas change) to the $K$ topic-term circles for term $w$.
\end{enumerate}
...
\item The width of the \textbf{Pink Bar} for topic $k$ and term $w$, denoted $P_{kw}$, is set to be proportional to $\phi_{kw} \times N_k$ for all topics $k = 1...K$ and terms $w = 1...W$.
\item The width of the \textbf{Gray Bar} for term $w$, denoted $G_w$, is set to be proportional to $M_w$, the user-supplied frequency of term $w$ across the entire corpus.
...
\begin{enumerate}
\item Note that the all the visual elements represent frequencies (of various things in the training data), rather than conditional probabilities. For example, the area of topic-term circle $A^\text{topic-term}_{wk}$ could have been set to be proportional to $\phi_{wk}/\sum_k \phi_{wk}$, but instead we set it to be proportional to $P_{wk}/\sum_k P_{wk}$. So, suppose the term ``foo" had a 0.003 probability under, say, topic 23 and topic 45, and negligible probability under all other topics. One might expect that upon highlighting ``foo", the topic-term circles would all disappear except for two equal-area topic-term circles representing topics 23 and 45. Instead, if, for example, topic 23 occurred twice as frequently as topic 45, then the topic-term circle for topic 23 would be twice as large as that for topic 45 upon ``foo" being highlighted. This reflects the fact that 2/3 of the occurrences of ``foo" in the training data were estimated to have come from topic 23.

The same principle holds for the pink bars and gray bars -- they visualize frequencies, rather than proportions, so that wider bars signify more frequent terms in the training data. We felt this was an important feature of the data to visualize, rather than building a visualization that simply displayed aspects of $\boldsymbol{\phi}$ and $\boldsymbol{\theta}$, which are normalized, and don't reflect the frequencies of the terms and topics in the data.
\item Note on priors and pseudocounts/pseudotokens...
\item Note on pixels: Default left panel is 530 x 530, and we set the sum of the areas of the default topic circles to be $530^2/3$, so that these circles cover at most 1/3 of the panel. The areas of the topic-term circles, on the other hand, often have a more skewed distribution, and we don't want a topic-term circle near the boundary of the left panel to get nearly 100\% of the total area of all the topic-term circles for a given term (and therefore spill paint outside the left panel); thus, we set the sum of their areas to be (at most) 25\% of the panel's area.
\end{enumerate}
